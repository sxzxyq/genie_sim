...
def create_drinks_group() -> Shape:
    ...
    Includes four bottles and one tumbler set for abundance.
    """
    # Bottled beverages
    ...
    brown_bottle = library_call(
        "usd",
        oid="genie_beverage_bottle_013",
        keywords=["brown_tea_bottle", "drink", "beverage", "left_back"]
    )
    
    black_bottle = library_call(
        "usd",
        oid="genie_beverage_bottle_009",
        keywords=["black_energy_drink", "drink", "beverage", "right_back"]
    )
    ...
    tumblers = library_call(
        ...
        keywords=["glass_tumblers", "drink", "glassware", "back_center"]
    )
    
    # Position all drinks around the meal
    red_bottle = transform_shape(red_bottle, translation_matrix([-0.25, 0.25, 0.0]))      # left-front
    white_bottle = transform_shape(white_bottle, translation_matrix([0.25, 0.25, 0.0]))   # right-front
    brown_bottle = transform_shape(brown_bottle, translation_matrix([-0.25, -0.25, 0.0])) # left-back
    black_bottle = transform_shape(black_bottle, translation_matrix([0.25, -0.25, 0.0]))  # right-back
    tumblers = transform_shape(tumblers, translation_matrix([0.0, -0.35, 0.0]))           # back-center
    
    return concat_shapes(red_bottle, white_bottle, brown_bottle, black_bottle, tumblers)
...
def abundant_fancy_meal_on_round_table() -> Shape:
    ...
    No robots or extra objects—only table, food, and drinks.
    """
    ...
    # Create and combine all edible and drinkable items
